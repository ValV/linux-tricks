...
```Логические тома внутри логической группы на одном физическом томе
 +------------------------------------+------------------------------------+
...
```Свободное место перед томом 1 за счёт размера этого тома
 +-----------------------+------------------------+------------------------+
...
Посмотреть, что в результате получилось, можно при помощи команды *pvdisplay*, т. е. показать информацию о физическом томе pv0 (который может быть представлен, например, ссылкой на sdb3: pv0 --> sdb3), на котором располагается группа томов vg0:
```
# pvdisplay -m /dev/pv0
